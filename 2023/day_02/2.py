...
for line in a_split:
    ...
    for i, subset in enumerate(sets):
        ...
        for item in subset.split(','):
            item = item.strip()
            if item.find('red') != -1:
                count_red = max(count_red, int(item.split(' ')[0]))
            if item.find('green') != -1:
                count_green = max(count_green, int(item.split(' ')[0]))
            if item.find('blue') != -1:
                count_blue = max(count_blue, int(item.split(' ')[0]))

    power_set = count_red * count_green * count_blue
    sum_power += power_set
    power_set = 0
    count_red = 0
    count_green = 0
